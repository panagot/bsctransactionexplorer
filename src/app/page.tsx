...
export default function Home() {
  ...
  const [transaction, setTransaction] = useState<BSCTransactionExplanation | null>(null);
  const [history, setHistory] = useState<BSCTransactionExplanation[]>([]);
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined') {
      const savedHistory = localStorage.getItem('bsc-tx-history');
      if (savedHistory) {
        try {
      ...
      const savedDarkMode = localStorage.getItem('bsc-dark-mode');
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined') {
      if (darkMode) {
        document.documentElement.classList.add('dark');
        localStorage.setItem('bsc-dark-mode', 'true');
      } else {
        document.documentElement.classList.remove('dark');
        localStorage.setItem('bsc-dark-mode', 'false');
      }
    }
  }, [darkMode]);
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const parsedTx = await parseBSCTransaction(txData, receipt);
      ...
      if (typeof window !== 'undefined') {
        localStorage.setItem('bsc-tx-history', JSON.stringify(newHistory));
      }
    } catch (err) {
    ...
    try {
      ...
      if (recentTxs && recentTxs.length > 0) {
        ...
        try {
          ...
          const parsedTx = await await parseBSCTransaction(txData, receipt);
  ...
  const exportToJSON = () => {
    ...
    link.href = url;
    link.download = `bsc-transaction-${transaction.hash}.json`;
    link.click();
  ...
  const shareTransaction = async () => {
    ...
    const shareData = {
      title: 'BSC Transaction Analysis',
      text: `Check out this BSC transaction: ${transaction.hash}`,
  ...
  const openInExplorer = () => {
    if (!transaction) return;
    window.open(`https://bscscan.com/tx/${transaction.hash}`, '_blank');
  };

  const getTransactionTypeColor = (type: string) => {
    switch (type) {
      case 'DEX_SWAP': return 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300';
      case 'BSC_TRANSFER': return 'bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-300';
      ...
      case 'PANCAKESWAP': return 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-300';
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-slate-600 dark:text-slate-400 text-sm">
                  An Easy to Read BSC Blockchain Explorer
                </p>
              </div>
            </div>
      ...
      <div className="flex-1 max-w-7xl mx-auto px-6 py-8 w-full">
        ...
        <div className="text-center mb-12">
          <h2 className="text-4xl font-bold text-slate-900 dark:text-white mb-4">
            Explore BSC Transactions
          </h2>
          <p className="text-xl text-slate-600 dark:text-slate-400 mb-8 max-w-2xl mx-auto">
            Enter any BSC transaction hash to get a detailed, easy-to-understand analysis with DeFi detection, balance changes, and educational insights.
          </p>
            ...
            <div className="flex gap-3">
              <div className="flex-1 relative">
                ...
                <input
                  ...
                  placeholder="Enter BSC transaction hash..."
                  className="w-full pl-12 pr-4 py-4 bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm border border-slate-200/50 dark:border-slate-700/50 rounded-2xl text-slate-900 dark:text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-yellow-500/50 focus:border-transparent transition-all duration-200"
                />
              </div>
              <button
                onClick={handleSearch}
                disabled={loading || !hash.trim()}
                className="px-8 py-4 bg-gradient-to-r from-yellow-500 to-orange-600 hover:from-yellow-600 hover:to-orange-700 disabled:from-slate-400 disabled:to-slate-500 text-white font-semibold rounded-2xl transition-all duration-200 flex items-center gap-2 shadow-lg hover:shadow-xl disabled:shadow-none"
              >
                {loading ? (
          ...
          <div className="mt-6 text-center">
            <div className="inline-flex items-center gap-2 px-3 py-1.5 bg-slate-100 dark:bg-slate-800 rounded-full">
              <span className="text-slate-600 dark:text-slate-400 text-xs">
                💡 <strong>Pro tip:</strong> Copy any transaction hash from <a href="https://bscscan.com" target="_blank" rel="noopener noreferrer" className="text-yellow-500 hover:underline font-medium">BscScan</a>
              </span>
            </div>
          </div>
        </div>
        ...
        {transaction && (
          <div className="space-y-6">
            ...
            <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-sm rounded-3xl shadow-2xl border border-slate-200 dark:border-slate-700 p-8 relative overflow-hidden">
              {/* Background Pattern */}
              <div className="absolute top-0 right-0 w-32 h-32 bg-gradient-to-br from-yellow-400/10 to-orange-400/10 rounded-full -translate-y-16 translate-x-16"></div>
              ...
              <div className="relative z-10">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="bg-slate-50/80 dark:bg-slate-800/50 rounded-2xl p-4">
                      ...
                      <p className="text-slate-600 dark:text-slate-400 text-sm leading-relaxed">
                        {transaction.transactionType === 'BSC_TRANSFER' 
                          ? `This transaction transferred ${transaction.valueInEth.toFixed(4)} BNB from one address to another. The transaction cost ${transaction.gasFee.toFixed(6)} BNB in gas fees, which covers network processing and validation.`
                          : transaction.transactionType === 'DEX_SWAP'
                          ? `You executed a token swap through a decentralized exchange. This transaction involved multiple token transfers and cost ${transaction.gasFee.toFixed(6)} BNB in gas fees. The swap was processed using BSC's smart contract system.`
                          : transaction.transactionType === 'PANCAKESWAP'
                          ? `You used PancakeSwap to swap tokens. This transaction involved PancakeSwap's smart contracts and cost ${transaction.gasFee.toFixed(6)} BNB in gas fees. PancakeSwap is the leading DEX on BSC.`
                          : `This transaction modified blockchain state and cost ${transaction.gasFee.toFixed(6)} BNB in gas fees. The cost covers computation and storage operations on the BSC network.`
                        }
                      </p>
                    </div>
                  </div>
                ...
                <div className="flex flex-wrap gap-3 pt-6 border-t border-slate-200 dark:border-slate-700">
                  ...
                  <button
                    ...
                    className="flex items-center gap-2 px-4 py-2.5 bg-yellow-100 dark:bg-yellow-900/30 hover:bg-yellow-200 dark:hover:bg-yellow-900/50 text-yellow-700 dark:text-yellow-300 rounded-xl transition-all duration-200 font-medium"
                  >
                  ...
                  <button
                    ...
                    className="flex items-center gap-2 px-4 py-2.5 bg-orange-100 dark:bg-orange-900/30 hover:bg-orange-200 dark:hover:bg-orange-900/50 text-orange-700 dark:text-orange-300 rounded-xl transition-all duration-200 font-medium"
                  >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-8">
            ...
            <div>
              ...
              <ul className="space-y-2 text-sm text-slate-600 dark:text-slate-400">
                ...
                <li>• DeFi protocol detection & analysis</li>
            ...
            <div>
              ...
              <ul className="space-y-2 text-sm text-slate-600 dark:text-slate-400">
                ...
                <li>• BSC Web3.js integration</li>
                <li>• EVM transaction parsing</li>
                <li>• PancakeSwap & DeFi analysis</li>
              </ul>
            </div>
          ...
          <div className="border-t border-slate-200/50 dark:border-slate-700/50 pt-6">
            <div className="flex flex-col md:flex-row items-center justify-between gap-4">
              ...
              <a 
                ...
                className="text-slate-600 dark:text-slate-400 hover:text-yellow-500 dark:hover:text-yellow-400 transition-colors"
